...
const PersonalMicroLoan = ({data}: any) => {
  ...
  useEffect(() => {
    ...
    const calculatedInterestSaved = baseInterest - (baseInterest * calculatedTerm) / baseTerm
    ...
    setPayoffDate(`${today.toLocaleString("default", { month: "short" })} ${today.getFullYear()}`)
  }, [paymentAmount])
  ...
  return (
    ...
    <PersonalMicroLoanInfo data = {data}/>
      ...
      <Tabs defaultValue="strategy" className="mb-8">
        ...
        <SimulatorTab paymentAmount = {paymentAmount} handlePaymentChange ={handlePaymentChange} handleInputChange = {handleInputChange} newTerm = {newTerm} monthsSaved = {monthsSaved} interestSaved = {interestSaved} percentSaved = {percentSaved} payoffDate = {payoffDate}/>
      </Tabs>
    </div>
    </section>
  )
